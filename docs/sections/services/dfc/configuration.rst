...
------------------------------
**StrictHostChecking** is SSH connection option. If it is enabled, checks HostName and public keys provided by server and comper it with keys stored locally. 
By default in DataFile Collector this option is enabled (true) and required provide known_hosts list to DFC containter.

**Important: DFC required public key in sha-rsa KeyAlgorithm** 
...
.. code-block:: bash
  ...
  kubectl -n <ONAP NAMESPACE> create cm <config map name> --from-file <path to known_hosts file inside kubernetes cluster>

2. Edit DFC deployment, add volume and volume monunts, **DFC deployment containt 3 containter, pay attention to mount the file to the appropriate container**
