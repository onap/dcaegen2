...
::
    ...
    <hv-ves launch command> --configuration-file "path/to/file"

- By defining environment variable

::

    export VESHV_CONFIGURATION_FILE=path/to/file
...
**HV-VES** looks for Consul under hostname "consul-server". If the service is down, above logs will appear and after few retries collector will shut down.
...
This log is printed when you upload a configuration in an invalid format (for example, with type in streams description).
Received json is not a valid Streams configuration, therefore **HV-VES** does not apply it and becomes **unhealthy**.
...
The above log is printed when the message payload size is too big. **HV-VES** does not handle messages that exceed specified payload size. It is equal to maximum value specified under streams_publishes configuration. If none are defined, default value **1048576 bytes (1MiB)** is used.
...
**server.p12** and **trust.p12** are default names of key and trust stores. They can be changed by specifying ``security.keys.trustStore`` or ``security.keys.keyStore`` file configuration entries.
