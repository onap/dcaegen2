...
.. csv-table::
    ...
    keyStorePasswordFile    ; String     ; Key store password file used in HV-VES incoming connections
    trustStoreFile          ; String     ; File with trusted certificate bundle in PKCS12 format path used in HV-VES incoming connections
    trustStorePasswordFile  ; String     ; Trust store password file used in HV-VES incoming connections

All security entries are mandatory with `sslDisable` set to `false`. Otherwise only `sslDisable` needs to be specified. If `sslDisable` flag is missing, then it is  interpreted same as it would be set to `false`.
