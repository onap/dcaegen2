...
Healthcheck
-----------
Inside HV-VES docker container runs a small HTTP service for healthcheck. Endpoint port for this service can be configured
at deployment using `--health-check-api-port` command line option or via environment variable (for details see :ref:`_deployment`).

This service exposes single endpoint **GET /health/ready** which returns a **HTTP 200 OK** when HV-VES is healthy
...
Prometheus endpoint share the same port with healthchecks.
